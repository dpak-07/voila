  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
      {sections.map((sec, idx) => {
        const Icon = sec.icon;
        return (
          <div key={idx} className={`p-5 rounded-2xl signal-card flex flex-col justify-between border ${sec.cardBorder} shadow-sm`}>
            <div>
              <div className="flex items-center justify-between pb-3 mb-3 border-b border-slate-200">
                <div className="flex items-center gap-2">
                  <span className={`p-1.5 rounded-lg ${sec.iconBg} shadow-2xs`}>
              ...
              {sec.items.length === 0 ? (
                <div className="py-8 px-4 text-center rounded-xl bg-slate-50 border border-dashed border-slate-300">
                  <p className="text-xs font-mono text-slate-500">{sec.emptyMsg}</p>
                </div>
              ) : (
                <div className="space-y-2.5">
                  {sec.items.slice(0, 4).map((item, itemIdx) => {
                    ...
                    return (
                      <div
                        key={itemIdx}
                        className="p-3.5 rounded-xl bg-slate-50/90 border border-slate-200 hover:border-slate-300 transition-colors shadow-2xs space-y-1.5"
                      >
                        <div className="flex items-start justify-between gap-2">
                          <span className="font-bold text-xs text-slate-900 capitalize leading-snug">
                            {name}
                          </span>
                          <span className={`px-2 py-0.5 rounded border text-[9px] font-mono font-bold uppercase shrink-0 ${sec.badgeBg}`}>
                            {sec.badge}
                          </span>
                        </div>

                        <div className="flex items-center gap-3 text-[11px] font-mono text-slate-500">
                          {volume && <span>Vol: <strong className="text-slate-900">{volume.toLocaleString()}</strong></span>}
                          {growth !== null && (
                            <span className="text-rose-600 font-bold">+{growth}% velocity</span>
                          )}
                        </div>
                      </div>
                    );
