...
export function TopBar({ onToggleSidebar, isSidebarCollapsed }) {
  ...
  return (
    <header className="h-16 px-4 sm:px-6 bg-white border-b border-slate-200/80 flex items-center justify-between sticky top-0 z-20">
      {/* Left: Sidebar Toggle + Active Run Selector & Slices */}
      <div className="flex items-center gap-2 sm:gap-2.5 min-w-0">
        {onToggleSidebar && (
          <button
            onClick={onToggleSidebar}
            className="p-1.5 rounded-lg text-slate-500 hover:text-slate-900 hover:bg-slate-100 transition-colors border border-slate-200 cursor-pointer shrink-0"
        ...
        <div className="relative min-w-0">
          <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-slate-50 hover:bg-slate-100/80 border border-slate-200 transition-colors">
            <Layers className="w-3.5 h-3.5 text-indigo-600 shrink-0" />
            <select
              value={activeRunId || 'all'}
              onChange={(e) => setActiveRunId(e.target.value)}
              className="bg-transparent text-xs font-sans text-slate-800 font-medium outline-none cursor-pointer pr-1 max-w-[160px] xs:max-w-[220px] sm:max-w-xs md:max-w-md truncate"
        ...
        <div className="relative shrink-0">
          <button
            ...
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-sans font-medium transition-colors border cursor-pointer ${
              hasActiveFilters
              ...
              <div className="space-y-3.5 font-mono text-xs max-h-88 overflow-y-auto pr-1">
                ...
                <div>
                  ...
                  <datalist id="company-dataset-suggestions">
                    {(dateRangeInfo?.available_companies || [])
                      .filter(c => c && !/^\d+$/.test(String(c).trim()))
                      .map((c) => (
      ...
      <div className="flex items-center gap-2 sm:gap-3 shrink-0">
        ...
        <button
          ...
          className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-white hover:bg-slate-50 text-slate-700 hover:text-slate-900 border border-slate-200 transition-colors text-xs font-medium cursor-pointer shadow-2xs"
        ...
        <button
          ...
          className="flex items-center gap-1.5 px-3.5 py-1.5 rounded-lg bg-indigo-600 hover:bg-indigo-700 text-white font-medium transition-all text-xs cursor-pointer shadow-2xs"
        >
